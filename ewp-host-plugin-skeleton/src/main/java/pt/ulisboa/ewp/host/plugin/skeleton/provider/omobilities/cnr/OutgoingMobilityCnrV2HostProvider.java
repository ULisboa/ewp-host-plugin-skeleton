...
/**
 * A provider that allows EWP Node to notify a host about a change notification on some outgoing
 * mobility kept by some partner institution (V2).
 */
public abstract class OutgoingMobilityCnrV2HostProvider extends HostVersionedProvider {

  private static final int DEFAULT_MAX_NUMBER_OUTGOING_MOBILITY_IDS_PER_REQUEST = 1;

  /**
   * Called when a change notification of an outgoing mobility emitted by one of partner
   * institutions.
   *
   * @see <a
   *     href="https://github.com/erasmus-without-paper/ewp-specs-api-omobility-cnr/tree/stable-v2">EWP
   *     Specification</a>
   * @param sendingHeiId Identifier of the HEI which is the sending partner of the outgoing
   *     mobilities that have been changed.
   * @param outgoingMobilityIds Outgoing Mobilities IDs that were changed.
   */
  public abstract void onChangeNotification(
